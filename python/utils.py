...
class EncoderContext:
    ...
    def __init__(self, decoder_hidden=None, decoder_input_concat=None, decoder_hidden_concat=None,
                 decoder_input_t0=None, attn_vec=None):
...
def get_logfiles(data_path, split=None):
	...
	"""
    return get_logfiles_with_gold_config(data_path=data_path, gold_configs_dir=None, split=split,
                                         with_gold_config=False)
...
def reformat(block):
    return {
        ...
        "type": color_regex.findall(str(block["Type"]))[0]
        # NOTE: DO NOT CHANGE! Unicode to str conversion needed downstream when stringifying the dict.
    }
...
def get_config_params(config_file):
    ...
    config_params = {}
    ignore_params = ['model_path', 'data_dir', 'log_step', 'epochs', 'stop_after_n', 'num_workers', 'seed',
                     'suppress_logs']
...
def get_next_actions_repr(next_actions, last_action, action_type_sensitive, use_condensed_action_repr=False):
    ...
    if not use_condensed_action_repr:
        next_placements_repr = get_one_hot_repr(next_placements, action_type_sensitive,
                                                action_type_null_case="placement")
        next_removals_repr = get_one_hot_repr(next_removals, action_type_sensitive, action_type_null_case="removal")
    else:
        next_placements_repr = get_condensed_repr(next_placements, last_action, action_type_sensitive,
                                                  action_type_null_case='placement')
        next_removals_repr = get_condensed_repr(next_removals, last_action, action_type_sensitive,
                                                action_type_null_case='removal')
...
if __name__ == "__main__":
    ...
    get_augmented_data_splits(
        "../data/augmented-no-spatial/logs/", "../data/augmented-no-spatial/gold-configurations/",
