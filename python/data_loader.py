...
if os.path.isdir('../build/install/Python_Examples/config_diff_tool'):
    # if running code from cwc-minecraft-models
    sys.path.append('../build/install/Python_Examples/config_diff_tool')
...
class CwCDataset(Dataset):
    ...
    def __init__(
            ...
            data_dir="../data/logs/", gold_configs_dir="../data/gold-configurations/",
            save_dest_dir="../data/saved_cwc_datasets", saved_dataset_dir="../data/saved_cwc_datasets",
            vocab_dir="../vocabulary/",
            encoder_vocab=None, decoder_vocab=None, dump_dataset=False, load_dataset=False, transform=None,
            sample_filters=[],
            add_augmented_data=False, augmented_data_fraction=0.0, aug_data_dir="../data/augmented-no-spatial/logs/",
            aug_gold_configs_dir="../data/augmented-no-spatial/gold-configurations/"
    ):
        """
        ...
        self.src_input_size_next_actions = (
                                               x_range + y_range + z_range if not self.use_condensed_action_repr else 3) + len(
            type2id) + (len(action2id) if self.action_type_sensitive else 0)
        ...
        if split == "train" and augment_dataset and augmentation_factor >= 1:
            ...
            if load_dataset:
                    ...
                    self.jsons += list(
                        map(
                            ...
                            map(
                                ...
                                map(
                                    reformat_utterances,
                                    get_logfiles_with_gold_config(aug_data_dir, aug_gold_configs_dir, split,
                                                                  from_aug_data=True)
                                )
                            )
                        )
                ...
                self.samples = self.process_samples(lower, compute_diff=self.compute_diff,
                                                    compute_perspective=self.compute_perspective)
    ...
    def set_args(self, num_prev_utterances=1, blocks_max_weight=1, use_builder_actions=False, num_next_actions=2,
                 include_empty_channel=False, use_condensed_action_repr=False, action_type_sensitive=False,
                 feasible_next_placements=False, spatial_info_window_size=1000, counters_extra_feasibility_check=False,
    ...
    def filter_augmented_samples(self):
        ...
        print('\nOriginal dataset contains', len(samples['orig']), 'original samples and', len(samples['aug']),
              'augmented samples (' + str(len(samples['orig']) + len(samples['aug'])), 'total samples).')

        if self.augmented_data_fraction > 0 and len(samples['aug']) == 0:
            print(
                'Error: you specified a fraction of augmented data, but the loaded dataset contains no augmented data.')
            sys.exit(0)
        ...
        if self.augmented_data_fraction < 1.0:
            ...
            chosen_aug_samples = np.random.choice(samples['aug'],
                                                  int(self.augmented_data_fraction * len(samples['aug'])),
                                                  replace=False)
                ...
                try:
                    ...
                    for i in range(1, len(world_states)):
                        ...
                        for k, curr_world_state in enumerate(reversed(world_states[:i + 1])):
                            ...
                            prev_blocks = [] if original_index == 0 else world_states[original_index - 1][
                                "BlocksInGrid"]
                        ...
                        if not last_world_state:
                            for i2 in range(len(observation["ChatHistory"])):
                                ...
                                for block in built_config:
                                    chat_with_actions_history.append(
                                        {"idx": i, "action": "putdown", "type": block["type"],
                                         "built_config": built_config, "prev_config": None,
                                         "builder_position": builder_position, "last_action": last_action,
                                         "gold_placement_list": gold_placement_list,
                                         "gold_removal_list": gold_removal_list})

                                chat_with_actions_history.append(
                                    {"idx": i, "action": "chat", "utterance": observation["ChatHistory"][i2].strip(),
                                     "built_config": built_config, "prev_config": None,
                                     "builder_position": builder_position, "last_action": last_action,
                                     "gold_placement_list": gold_placement_list,
                                     "gold_removal_list": gold_removal_list})
                            ...
                            delta = {"putdown": config_diff["gold_minus_built"],
                                     "pickup": config_diff["built_minus_gold"]}

                            for action_type in delta:
                                for block in delta[action_type]:
                                    chat_with_actions_history.append(
                                        {"idx": i, "action": action_type, "type": block["type"],
                                         "built_config": built_config, "prev_config": prev_config,
                                         "builder_position": builder_position, "last_action": last_action,
                                         "gold_placement_list": gold_placement_list,
                                         "gold_removal_list": gold_removal_list})
                            ...
                            if len(observation["ChatHistory"]) > len(last_world_state["ChatHistory"]):
                                for i3 in range(len(last_world_state["ChatHistory"]), len(observation["ChatHistory"])):
                                    ...
                                    chat_with_actions_history.append({"idx": i, "action": "chat",
                                                                      "utterance": observation["ChatHistory"][
                                                                          i3].strip(), "built_config": built_config,
                    ...
                    for i in range(len(chat_with_actions_history)):
                        ...
                        gold_placement_list = [append_block_perspective_coords(builder_position, block) for block in
                                               elem["gold_placement_list"]]
                        gold_removal_list = [append_block_perspective_coords(builder_position, block) for block in
                                             elem["gold_removal_list"]]
                        ...
                        for k in range(i):
                            ...
                            if prev_elem['action'] != 'chat':
                                prev_utterances.append({'speaker': 'Builder', 'utterance': [
                                    '<builder_' + prev_elem['action'] + '_' + prev_elem['type'] + '>']})
                                ...
                                prev_utterance = prev_utterance[
                                                 len(architect_prefix):] if prev_speaker == 'Architect' else prev_utterance[
                                                                                                             len(builder_prefix):]
                        ...
                        if compute_diff:
                            gold_v_built_diff, perturbations_and_diffs = get_diff(gold_config=gold_config,
                                                                                  built_config=built_config)

                            # get type distributions
                            diffs_built_config_space = list(
                                map(lambda x: x.diff.diff_built_config_space, perturbations_and_diffs))
                            type_distributions_built_config_space = reformat_type_distributions(
                                get_type_distributions(diffs_built_config_space=diffs_built_config_space,
                                                       built_config=built_config)
                            )

                            # reverse diff
                            _, perturbations_and_diffs_reverse = get_diff(gold_config=built_config,
                                                                          built_config=gold_config)

                            # get type distributions
                            diffs_gold_config_space = list(
                                map(lambda x: x.diff.diff_built_config_space, perturbations_and_diffs_reverse))
                            type_distributions_gold_config_space = reformat_type_distributions(
                                get_type_distributions(diffs_built_config_space=diffs_gold_config_space,
                                                       built_config=gold_config)
                            )

                        perspective_coordinates = None if not compute_perspective else get_perspective_coord_repr(
                            builder_position)
                        ...
                        samples.append(
                            {
                                ...
                                # all diffs based on all optimal alignments -- in the built config space
                                'json_id': j,  # ID of the json this sample was obtained from
                                'sample_id': idx,  # ID assigned to this sample
                                'next_utterance_raw': next_utterance
                                # raw format of the utterance to be predicted -- for downstream purposes
    ...
    def __getitem__(self, idx):
        ...
        next_utterance_input.append(
            self.decoder_vocab('<architect>'))  # NOTE: no need to change for LM using builder utterances too
        ...
        for prev in utterances_to_add:
            ...
            if "<dialogue>" in utterance[0]:
                ...
                start_token = self.encoder_vocab('<architect>') if 'Architect' in speaker else self.encoder_vocab(
                    '<builder>')
                end_token = self.encoder_vocab('</architect>') if 'Architect' in speaker else self.encoder_vocab(
                    '</builder>')
        ...
        built_config_3d_repr = get_3d_repr(built_config, max_weight=self.blocks_max_weight,
                                           include_empty_channel=self.include_empty_channel) if self.model == 'cnn_3d' else None
        ...
        gold_config_3d_repr = get_3d_repr(gold_config,
                                          include_empty_channel=self.include_empty_channel) if self.model == 'cnn_3d' else None
        ...
        next_actions_gold = {"gold_minus_built": sample["gold_placement_list"][:int(self.num_next_actions / 2)],
                             "built_minus_gold": sample["gold_removal_list"][:int(self.num_next_actions / 2)]}
        next_actions_gold_repr = get_next_actions_repr(next_actions_gold, last_action,
                                                       action_type_sensitive=self.action_type_sensitive,
                                                       use_condensed_action_repr=self.use_condensed_action_repr)
        ...
        if diff and self.model == 'utterances_and_next_actions':
            next_actions = get_next_actions(all_next_actions=diff, num_next_actions_needed=self.num_next_actions,
                                            last_action=last_action, built_config=built_config,
                                            feasible_next_placements=self.feasible_next_placements)
            next_actions['gold_minus_built'] = [append_block_perspective_coords(builder_position, block) for block in
                                                next_actions['gold_minus_built']]
            next_actions['built_minus_gold'] = [append_block_perspective_coords(builder_position, block) for block in
                                                next_actions['built_minus_gold']]
            next_actions_repr = get_next_actions_repr(next_actions, last_action,
                                                      action_type_sensitive=self.action_type_sensitive,
                                                      use_condensed_action_repr=self.use_condensed_action_repr)

        # block global counters
        diffs_built_config_space = sample["diffs_built_config_space"]
        block_counters = get_block_counters(diffs_built_config_space, built_config=built_config,
                                            built_config_in_region=built_config,
                                            extra_check=self.counters_extra_feasibility_check) if diff else None

        # block region counters
        block_counters_spatial_info = get_block_counters_spatial_info(diffs_built_config_space, built_config,
                                                                      last_action, builder_position,
                                                                      window_size=self.spatial_info_window_size,
                                                                      extra_check=self.counters_extra_feasibility_check) if diff else None
        ...
        if block_counters_spatial_info:
            if self.use_existing_blocks_counter:
                block_counters_spatial_tensors = [(torch.Tensor(x.block_counters.all_placements_counter),
                                                   torch.Tensor(x.block_counters.all_next_placements_counter),
                                                   torch.Tensor(x.block_counters.all_removals_counter),
                                                   torch.Tensor(x.block_counters.all_existing_blocks_counter)) for x in
                                                  block_counters_spatial_info]
            else:
                block_counters_spatial_tensors = [(torch.Tensor(x.block_counters.all_placements_counter),
                                                   torch.Tensor(x.block_counters.all_next_placements_counter),
                                                   torch.Tensor(x.block_counters.all_removals_counter)) for x in
                                                  block_counters_spatial_info]
        ...
        return (
            ...
            torch.Tensor(block_counters.all_next_placements_counter) if block_counters else None,
            # global block counters
            ...
            RawInputs(next_actions, next_actions_gold, json_id=sample.get('json_id'), sample_id=sample.get('sample_id'),
                      next_utterance_raw=sample.get('next_utterance_raw'), built_config_ss=built_config,
                      gold_config_ss=gold_config, colors_to_all_actions=colors_to_all_actions),
            # raw inputs for downstream purposes
    ...
    def collate_fn(self, data):
        ...
        prev_utterances, built_configs, gold_configs, target_inputs, target_outputs, next_placements, next_removals, gold_placements, gold_removals, all_placements_counter, all_next_placements_counter, all_removals_counter, block_counters_spatial_tensors, last_action_bits, raw_inputs, built_configs_3d, gold_configs_3d, perspective_coord_reprs, built_config_type_dists, gold_config_type_dists = zip(
            *data)
        ...
        if self.use_existing_blocks_counter:
            block_counters_spatial_tensors = [(w.unsqueeze(0), x.unsqueeze(0), y.unsqueeze(0), z.unsqueeze(0)) for
                                              (w, x, y, z) in block_counters_spatial_tensors]
        else:
            block_counters_spatial_tensors = [(x.unsqueeze(0), y.unsqueeze(0), z.unsqueeze(0)) for (x, y, z) in
        ...
        return (
            EncoderInputs(prev_utterances, prev_utterances_lengths, built_configs, built_config_lengths, gold_configs,
                          gold_config_lengths, next_actions, next_actions_lengths, gold_actions, gold_actions_lengths,
                          block_counters, block_counters_spatial_tensors, last_action_bits, built_configs_3d,
                          gold_configs_3d, perspective_coord_reprs, built_config_type_dists, gold_config_type_dists),
    ...
    def augment_dataset(self, k, vocab_dir="../vocabulary/"):
        ...
        with open(
                vocab_dir + "/synthetic_utterances" + split + lower + add_builder_utterances + augmentation_factor + ".txt",
                'w') as f:
    ...
    def get_new_examples(self, tokenized_utterance, k):
        ...
        new_examples = [list(x) for x in
                        set(tuple(x) for x in new_examples)]  # select only unique new synthetic utterances
...
class EncoderInputs:
    def __init__(self, prev_utterances, prev_utterances_lengths, built_configs, built_config_lengths, gold_configs,
                 gold_config_lengths, next_actions, next_actions_lengths, gold_actions, gold_actions_lengths,
                 block_counters, block_counters_spatial_tensors, last_action_bits, built_configs_3d, gold_configs_3d,
                 perspective_coord_reprs, built_config_type_dists, gold_config_type_dists):
...
class RawInputs:
    ...
    def __init__(self, next_actions_raw, gold_next_actions_raw, json_id=None, sample_id=None, next_utterance_raw=None,
                 built_config_ss=None, gold_config_ss=None, colors_to_all_actions=None):
...
class Region:
    ...
    def set_block_counters(self, diffs_built_config_space, built_config, extra_check):
        ...
        # get block counters
        self.block_counters = get_block_counters(region_diffs, built_config=built_config,
                                                 built_config_in_region=built_config_in_region, extra_check=extra_check)
    ...
    def get_region_id(self, builder_position, last_action):
        ...
        if diff_vector["x"] != 0 and diff_vector["y"] != 0 and diff_vector["z"] != 0:
            ...
            self.region_id = direction_to_id[
                stringify((diff_vector_to_direction[signed_x], diff_vector_to_direction[signed_y],
                           diff_vector_to_direction[signed_z]))
            ]
...
all_directions = [
                     null] + lr + td + fb + lr_rows + td_columns + fb_rows + lr_td + td_fb + fb_lr + lr_td_fb  # NOTE: DO NOT CHANGE THIS ORDERING WITHOUT THOUGHT!
...
def get_block_counters_spatial_info(diffs_built_config_space, built_config, last_action, builder_position, window_size,
                                    extra_check):
    """
    ...
    adjacent_regions = sorted(adjacent_regions,
                              key=lambda x: x.get_region_id(builder_position, last_action))  # mutating
...
def get_adjacent_regions(action, window_size):
    ...
    action_cell = Region(x_min=action["x"], x_max=action["x"], y_min=action["y"], y_max=action["y"], z_min=action["z"],
                         z_max=action["z"])

    if window_size >= 1:
        cells = [
            Region(x_min=action["x"] + 1, x_max=action["x"] + 1, y_min=action["y"], y_max=action["y"],
                   z_min=action["z"], z_max=action["z"]),
            Region(x_min=action["x"] - 1, x_max=action["x"] - 1, y_min=action["y"], y_max=action["y"],
                   z_min=action["z"], z_max=action["z"]),
            Region(x_min=action["x"], x_max=action["x"], y_min=action["y"] + 1, y_max=action["y"] + 1,
                   z_min=action["z"], z_max=action["z"]),
            Region(x_min=action["x"], x_max=action["x"], y_min=action["y"] - 1, y_max=action["y"] - 1,
                   z_min=action["z"], z_max=action["z"]),
            Region(x_min=action["x"], x_max=action["x"], y_min=action["y"], y_max=action["y"], z_min=action["z"] + 1,
                   z_max=action["z"] + 1),
            Region(x_min=action["x"], x_max=action["x"], y_min=action["y"], y_max=action["y"], z_min=action["z"] - 1,
                   z_max=action["z"] - 1)
        ]
    ...
    if window_size >= 2:
        rows_and_columns = [
            Region(x_min=action["x"] + 2, x_max=action["x"] + window_size, y_min=action["y"], y_max=action["y"],
                   z_min=action["z"], z_max=action["z"]),
            Region(x_min=action["x"] - window_size, x_max=action["x"] - 2, y_min=action["y"], y_max=action["y"],
                   z_min=action["z"], z_max=action["z"]),
            Region(x_min=action["x"], x_max=action["x"], y_min=action["y"] + 2, y_max=action["y"] + window_size,
                   z_min=action["z"], z_max=action["z"]),
            Region(x_min=action["x"], x_max=action["x"], y_min=action["y"] - window_size, y_max=action["y"] - 2,
                   z_min=action["z"], z_max=action["z"]),
            Region(x_min=action["x"], x_max=action["x"], y_min=action["y"], y_max=action["y"], z_min=action["z"] + 2,
                   z_max=action["z"] + window_size),
            Region(x_min=action["x"], x_max=action["x"], y_min=action["y"], y_max=action["y"],
                   z_min=action["z"] - window_size, z_max=action["z"] - 2)
        ]
    else:
        rows_and_columns = []
...
def get_block_counters(diffs_built_config_space, built_config, built_config_in_region, extra_check):
	...
	"""
    counters_per_diff = list(
        map(lambda x: region_to_counters(x, built_config, built_config_in_region, extra_check).__dict__,
            diffs_built_config_space))

    results = BlockCounters(None, None, None, None)  # stores final result to return

    for field in ["all_placements_counter", "all_next_placements_counter", "all_removals_counter",
                  "all_existing_blocks_counter"]:
...
def region_to_counters(a_diff, built_config, built_config_in_region, extra_check):
    ...
    all_next_placements = list(
        filter(lambda x: is_feasible_next_placement(x, built_config, extra_check), all_placements))
    ...
    return BlockCounters(counts_all_placements, counts_all_next_placements, counts_all_removals,
                         counts_all_existing_blocks)
...
class BlockCounters:
    ...
    def __init__(self, all_placements_counter, all_next_placements_counter, all_removals_counter,
                 all_existing_blocks_counter):
...
def reorder(observations):
    ...
    for i, state in enumerate(observations["WorldStates"]):
        ...
        observations["WorldStates"][i][
            "BlocksInGrid"] = curr_blocks_reordered  # mutate - will be used in next iteration
...
if __name__ == '__main__':
    ...
    parser.add_argument('--add_augmented_data', default=False, action='store_true',
                        help='add dialog-level augmented dataset')
    parser.add_argument('--ignore_diff', default=False, action='store_true', help='skip computing diff')
    parser.add_argument('--ignore_perspective', default=False, action='store_true',
                        help='skip computing perspective coordinates')
    ...
    parser.add_argument('--saved_dataset_dir', default="../data/saved_cwc_datasets/lower-no_perspective_coords/",
                        help='location of saved dataset')
    parser.add_argument('--num_prev_utterances', type=int, default=5,
    ...
    parser.add_argument('--spatial_info_window_size', type=int, default=1000,
                        help='3d window size to extract spatial information from')
    parser.add_argument('--use_existing_blocks_counter', default=False, action='store_true',
                        help='include counters for existing blocks')
    ...
    dataset = CwCDataset(
        model=args.model, split=args.split, lower=args.lower, add_builder_utterances=args.add_builder_utterances,
        compute_diff=not args.ignore_diff, compute_perspective=not args.ignore_perspective,
        encoder_vocab=encoder_vocab, decoder_vocab=decoder_vocab, dump_dataset=args.dump_dataset,
        load_dataset=args.load_dataset,
        saved_dataset_dir=args.saved_dataset_dir, transform=None, sample_filters=[],
        add_augmented_data=args.add_augmented_data, augmented_data_fraction=args.augmented_data_fraction
    )

    dataset.set_args(num_prev_utterances=args.num_prev_utterances, blocks_max_weight=args.blocks_max_weight,
                     use_builder_actions=args.use_builder_actions, num_next_actions=args.num_next_actions,
