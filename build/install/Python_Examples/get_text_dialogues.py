...
def generateTxtFile(logfiles, output, screenshots_dir):
    ...
    outfile_no_actions = open(
        "dialogues/" + ".".join(output.split(".")[:-1]) + ("-" if not output_is_empty else "") + "dialogue.txt", "w")
    outfile_with_actions = open("dialogues/" + ".".join(output.split(".")[:-1]) + (
        "-" if not output_is_empty else "") + "dialogue-with-actions.txt", "w")
    ...
    if not with_actions:
        ...
        for world_state in world_states:
            ...
            if "-chat" not in builder_ss and "-chat" not in architect_ss:
                fixed_viewers_ss = getFixedViewerScreenshots(screenshots_dir, experiment_name, screenshots,
                                                             observations["NumFixedViewers"])
            ...
            if action is None:
                ...
                if block is not None:
                    ...
                    bx, by, bz = str(block["AbsoluteCoordinates"]["X"]), str(block["AbsoluteCoordinates"]["Y"]), str(
                        block["AbsoluteCoordinates"]["Z"])
                    dialogue.append("[Builder " + (
                        "picks up" if "pickup" in action else "puts down") + " a " + btype + " block at X:" + bx + " Y:" + by + " Z:" + bz + "]")
...
def getMovedBlock(last_world_state, world_state, action):
    def containsBlock(last_world_state, block):
        for previous_block in last_world_state["BlocksInGrid"]:
            px, py, pz = previous_block["AbsoluteCoordinates"]["X"], previous_block["AbsoluteCoordinates"]["Y"], \
            previous_block["AbsoluteCoordinates"]["Z"]
            ptype = previous_block["Type"]

            if px == block["AbsoluteCoordinates"]["X"] and py == block["AbsoluteCoordinates"]["Y"] and pz == \
                    block["AbsoluteCoordinates"]["Z"] and ptype == block["Type"]:
                return True
...
def getFixedViewerScreenshots(screenshots_dir, experiment_name, screenshots, num_fixed_viewers):
    ...
    for i in range(num_fixed_viewers):
        fixed_viewers_ss.append(
            getScreenshotFilePath(screenshots_dir, experiment_name, screenshots.get("FixedViewer" + str(i + 1))))
...
def main():
    parser = argparse.ArgumentParser(
        description="Produce .txt file of dialogues from given json logfiles or directories.")
    parser.add_argument('-l', '--list', nargs='+',
                        help='Json files to be processed, or the directory in which they live', required=True)
    parser.add_argument('-o', '--output', help='Name of output txt file', required=True)
    parser.add_argument('-s', '--screenshots_dir', default=None,
                        help="Screenshots directory path. If unspecified, retrieves screenshots from a default location determined from the logfiles path")
